...
# Аудиоресурсы
const SFX_JUMP = preload("res://assets/audio/sfx/jump.ogg")
const SFX_DUCK = preload("res://assets/audio/sfx/duck.ogg")
const SFX_COLLECT_EGG = preload("res://assets/audio/sfx/collect_egg.ogg")
# collect_seed будет загружаться динамически, так как может быть проблема с файлом
var SFX_COLLECT_SEED: AudioStream = null
const SFX_HIT = preload("res://assets/audio/sfx/hit.ogg")
const SFX_GAME_OVER = preload("res://assets/audio/sfx/game_over.ogg")
const MUSIC_BG = preload("res://assets/audio/music/bg_music.mp3")

# AudioStreamPlayer узлы (создаются программно)
...
func _init_audio():
	...
	sound_enabled = GameManager.sound_enabled
	music_enabled = GameManager.music_enabled
	...
	print("Music player created, stream: ", MUSIC_BG.resource_path if MUSIC_BG else "null")
	
	# Создаем AudioStreamPlayer для звуковых эффектов
	sfx_player = AudioStreamPlayer.new()
	sfx_player.volume_db = 0.0
	add_child(sfx_player)
	print("SFX player created")
	
	# sfx_seed_player больше не нужен, так как последовательность звуков убрана
	# Можно удалить эту переменную, но оставим для совместимости
	# Загружаем collect_seed при инициализации (если файл существует)
	_load_collect_seed()
	
	# Проверяем загрузку всех звуков
	print("Sound files check:")
	print("  SFX_JUMP: ", SFX_JUMP != null)
	print("  SFX_DUCK: ", SFX_DUCK != null)
	print("  SFX_COLLECT_EGG: ", SFX_COLLECT_EGG != null)
	print("  SFX_COLLECT_SEED: ", SFX_COLLECT_SEED != null)
	print("  SFX_HIT: ", SFX_HIT != null)
	print("  SFX_GAME_OVER: ", SFX_GAME_OVER != null)
	print("  MUSIC_BG: ", MUSIC_BG != null)
	
	# Музыка запустится в start_game() когда игра начнется
...
func _on_pause_resume_pressed():
	...
	if music_player:
		...
		# Если музыка не играет, запускаем заново
		if not music_player.playing and music_enabled:
			music_player.play()
...
func _on_pause_sound_pressed():
	...
	GameManager.set_sound_enabled(sound_enabled)  # Сохраняем в GameManager
...
func _on_pause_music_pressed():
	...
	GameManager.set_music_enabled(music_enabled)  # Сохраняем в GameManager
	# Обновляем визуальное состояние кнопки (как в меню)
	if pause_music_button:
		if music_enabled:
	...
	if music_player:
		if music_enabled and game_started and not is_paused:
			music_player.play()
		else:
			music_player.stop()
...
func lose_life():
	...
	# Воспроизводим звук удара с увеличенной громкостью
	play_sfx_louder(SFX_HIT, 5.0)  # Увеличиваем громкость на 5 дБ
...
func _on_game_over_home_pressed():
	# Возврат в меню
	get_tree().paused = false
	# Музыка уже остановлена в game_over()
	get_tree().change_scene_to_file("res://scenes/Menu.tscn")
...
func _on_game_over_music_pressed():
	...
	# Управляем воспроизведением музыки (но не запускаем, так как игра окончена)
	if music_player:
...
func play_sfx(stream: AudioStream):
	if sound_enabled and sfx_player and stream:
		sfx_player.stream = stream
		sfx_player.play()
		print("Playing SFX: ", stream.resource_path if stream else "null")
	else:
		if not sound_enabled:
			print("SFX disabled, not playing sound")
		if not sfx_player:
			print("SFX player not initialized")
		if not stream:
			print("Stream is null, cannot play sound")

# Функция для воспроизведения звука с увеличенной громкостью
func play_sfx_louder(stream: AudioStream, volume_db_increase: float = 5.0):
	if sound_enabled and sfx_player and stream:
		# Сохраняем текущую громкость
		var original_volume = sfx_player.volume_db
		# Увеличиваем громкость
		sfx_player.volume_db = original_volume + volume_db_increase
		sfx_player.stream = stream
		sfx_player.play()
		# Восстанавливаем громкость через небольшую задержку (примерно 0.2 секунды)
		_restore_sfx_volume_after_delay(original_volume, 0.2)
		print("Playing SFX louder: ", stream.resource_path if stream else "null", " (volume: ", original_volume + volume_db_increase, " dB)")
	else:
		if not sound_enabled:
			print("SFX disabled, not playing sound")
		if not sfx_player:
...
func _restore_sfx_volume_after_delay(original_volume: float, delay: float):
	# Восстанавливаем громкость через задержку
	await get_tree().create_timer(delay).timeout
	if sfx_player:
		sfx_player.volume_db = original_volume

# Функции для воспроизведения конкретных звуков (вызываются из других скриптов)
func play_jump_sfx():
	play_sfx(SFX_JUMP)
...
func play_collect_egg_sfx():
	play_sfx(SFX_COLLECT_EGG)

func _load_collect_seed():
	# Файл collect_seed.ogg не может быть загружен (возможно поврежден или неправильно импортирован)
	# Используем collect_egg как запасной вариант, чтобы игра не падала
	print("WARNING: collect_seed.ogg cannot be loaded, using collect_egg.ogg instead")
	SFX_COLLECT_SEED = SFX_COLLECT_EGG

func play_collect_seed_sfx():
	# Если файл еще не загружен, пытаемся загрузить
	if not SFX_COLLECT_SEED:
		_load_collect_seed()
	
	# Просто проигрываем звук сбора семечка (без последовательности)
	play_sfx(SFX_COLLECT_SEED)
